...
var negative = -6;
var notANumber = 4 * "5";
var bigNumber = [8, 3, 11, 23, 21, 31, 42, 52, 33, 22, 44, 1, 2, 3, 55, 88];
console.log(
...
console.log("You can see what this does: ", bigNumber);
...
// PUT YOUR CODE HERE
let loveCoding = true;
let codingIsEasy = false;

console.log("The variable I made true is: ", loveCoding);
//The output of the above should be true.
console.log("The variable I made false is: ", codingIsEasy);
...
let tempLitString = `Hello! My name is ${firstName} ${lastName} and I was born in ${country}.`;
console.log(tempLitString);
...
var floatInteger = floatingPoint + integer;
console.log(floatInteger);

var intNeg = integer + negative;
console.log(intNeg);
...
var billTotal = 11.56;
var tipPercentage = 0.7;
var tip = billTotal * tipPercentage;
billTotal += tip;

console.log("The new bill total with tip is ", billTotal);
// Don't forget your console.logs!
...
//I picked Q#15 because I am not even sure I got the  question actually answered correctly.  I wanted to practice multiplying different things making sure it is correct.  I declared my variables using a floating-point number and an integer.
...
console.log("Q8 first larger: ", absDiff(29, 5));
console.log("Q8 second larger: ", absDiff(3, 16));
console.log("Q8 same nums: 0: ", absDiff(5, 5));
console.log("Q8 neg num: ", absDiff(-6, 5));
// ---------- QUESTION 24. ----------
...
const biggestStringLength = (string1, string2) => {
...
console.log(biggestStringLength(word1, word2));
...
// PUT YOUR CODE HERE
const prompt = require("prompt-sync")();
const birthday = prompt("enter the date of your next birthday (YYYY-MM-DD): ");

function birthdayCountdown(birthday) {
  let birthdayDate = new Date(birthday);
  let today = new Date();
  birthdayDate.setFullYear(
    today.getFullYear() + (birthdayDate < today ? 1 : 0),
  );
  return Math.ceil((birthdayDate - today) / (1000 * 60 * 60 * 24));
}
console.log(
  `This is how many days until your next birthday: ${birthdayCountdown(birthday)}`,
);
//
// ---------- QUESTION 26. ----------
...
const timeDifference = (start, end) => {
  const toMinutes = (time) =>
    time.split(":").reduce((acc, curr) => acc * 60 + Number(curr), 0);
  let diff = toMinutes(end) - toMinutes(start);
  return `${Math.floor((diff + (diff < 0 ? 1440 : 0)) / 60)} hour(s) and ${(diff + (diff < 0 ? 1440 : 0)) % 60} minute(s)`;
};

let startTime = prompt("Enter start time (HH:MM): ");
let endTime = prompt("Enter end time (HH:MM): ");

console.log(`Time difference: ${timeDifference(startTime, endTime)}`);
...
// PUT YOUR CODE HERE
//I had a very hard time with Q25.
//I am not even sure if this will work.  I cannot get codes to run past 25.  I cannot figure out what is wrong with Q25
const prompt = require("prompt-sync")();

const monthCountdown = (month) => {
  ...
  return `${diff} day(s) until the start of month ${month}!`;
};
const month = prompt("Enter a month (1-12): ");

console.log(monthCountdown(month));

// Don't forget your console.logs!
//I picked Q25 because it is giving me the most trouble
