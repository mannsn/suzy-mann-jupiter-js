...
function isDog(aPet){
  ...
  if (aPet.species === speciesChecker)
    return true;
  else
    return false;
}
...
function sortArray(libraryArray) {
  ...
  sortedArray.sort(function (a,b) {return a.libraryID-b.libraryID});
...
function addTypePropertywithMap(anArray){

  const newArray = anArray.map((addType) => 
    Object.assign({}, addType, {type:"book"})
  );
 
 
  return newArray;
}
 console.log("Q6 map: ", addTypePropertywithMap(library));
